...
- クッキー (Cookie)
    - クッキーとは
        - クッキーはユーザのブラウザ上に保存されるファイルで、
            - クッキーが送信されたサーバの名前（違うサーバに送るのを防ぐため）
            - クッキーの有効期限
            - 値 (ユーザの情報)
            が保存される。
    - クッキーの種類
        - **セッションクッキー**
            - Webサイトにアクセスしている間一時的に保存され、離れると消える
        - **永続クッキー**
            - 離れても残る。再度アクセス時にアクティブになる
    - 最近、、、
        - GDPR(General Data Protection Regularion) EU一般データ保護規則が施行され、データの取り扱いが厳重化された。
        - (おそらく世界一データを扱っている)Google は、Google Analytics を各サイトで使用する場合は、サイトポリシーにクッキーを使用していることを明言しなければならないとしている。
- セッション
    - セッションとは
        - サーバ上に保存されたデータで、ユーザが Web サイト上にいるとき、常に利用できる。
        - セッションは活動内容(ユーザのアクション)を記憶するもの
    - 最近、、、
        - タブ型ブラウザ(多分現行のブラウザ全般をさす)では、同じＰＣ上ではタブやウィンドウを複数表示させてもセッションは共有される
        - 
- 参考リンク
    - [クッキー（Cookie）とセッションとキャッシュの違いは何か？](https://ssaits.jp/promapedia/technology/cookie-session-cache.html)
    - [Qiita](https://qiita.com/hththt/items/07136ad74127999df271)